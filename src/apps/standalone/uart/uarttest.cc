...
int main() {
    ...
    uint8_t txdata_char_help[] = "\nhelp\r";
    uint8_t txdata_char_list[] = "\nlist\r";
    uint8_t txdata_char_read[] = "\nread 1KH1V.TXT\r";
    uint8_t txdata_char_unknown[] = "\nUnknown command\r";
        ...
        if (num_recv_bytes) {
            ...
            if (num_recv_bytes == 1) {
                if (rx_buf[0] == 'q') {
                    ...
                    uart_fifo_write(txdata_char_unknown, strlen((const char*)txdata_char_unknown));
                    continue;
                }
            }
